...
# characters received are jpqm, which are all different. In this case, your subroutine should report the value 7, 
...
pzl_data <- get_pzl_data(6, year = 2022)

# Solution 1/2 ----------------------------------------------------------

stream <- strsplit(pzl_data,"")[[1]]

code_l <- 4

markers <- sapply(c(1:(length(stream) - (code_l - 1))), function(x, stream_buffer = stream) {
  
  eval_buffer <- c(stream_buffer[c(x:(x + code_l - 1))])
  
  if (length(unique(eval_buffer)) == code_l) {
    return(x + (code_l - 1))
  } else {
    return(0)
  }
...
markers2 <- sapply(c(1:(length(stream) - (code_l - 1))), function(x, stream_buffer = stream) {
