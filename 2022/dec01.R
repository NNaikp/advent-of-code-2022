...
pzl_data <- get_pzl_data(1, year = 2022)

# Solution 1/2 ----------------------------------------------------------

# Split the vector
elfs <- split(pzl_data,cumsum(match(pzl_data,"", nomatch = 0))) %>% 
  lapply(., function(x) {sum(as.integer(x[x != ""]))}) %>% 
  unlist(.)
...
sol2 <- sum(sort(elfs,decreasing = TRUE)[1:3])
