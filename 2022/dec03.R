...
# A given rucksack always has the same number of items in each of its two compartments, so 
# the first half of the characters represent items in the first compartment, while the second 
...
pzl_data <- get_pzl_data(3, year = 2022)

# Solution 1/2 ----------------------------------------------------------

aBc <- c(letters,LETTERS)

items <- sapply(pzl_data, function(x) {
  l_char <- nchar(x)
  
  st1 <- substr(x, 1, l_char/2)
  st2 <- substr(x, l_char/2 + 1, l_char)
  
  u_char <- unique(strsplit(st1,"")[[1]])
  
  val <- match(u_char[str_which(st2, u_char)], aBc)
...
items <- sapply(seq(1, length(pzl_data), by = 3), function(x) {
  ...
  st1 <- u_char[str_which(pzl_data[c(x + 1)], u_char)]
  st2 <- u_char[str_which(pzl_data[c(x + 2)], u_char)]
  
  frq <- table(rle(c(st1,st2)))
  
  val <- match(names(frq[1,])[frq[1,] > 1], aBc)
