...
pzl_data <- get_pzl_data(4, year = 2022)

# Solution 1/2 ----------------------------------------------------------

all_elfs <- sapply(pzl_data, function(x) {
  
  clean <- strsplit(x,",")[[1]] %>% 
    gsub("-",":",.)
  
  clean1 <- eval(parse(text = clean[1]))
  clean2 <- eval(parse(text = clean[2]))
  
  # We always want to compare shortest vs longest
  if (length(clean1) <= length(clean2)) {
    first <- clean1
    second <- clean2
  } else {
    first <- clean2
  ...
  all_overlap <- all(!is.na(match(first, second)))
...
all_elfs2 <- sapply(pzl_data, function(x) {
  ...
  all_overlap <- any(!is.na(match(first, second)))
