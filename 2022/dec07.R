...
# To do this, you need to determine the total size of each directory. The total size of a directory is the sum of the sizes of the files it contains, directly or indirectly. (Directories themselves do not count as having any intrinsic size.)
# 
...
pzl_data <- get_pzl_data(7, year = 2022)

# Solution 1/2 ----------------------------------------------------------

files <- grepl("^[0-9]", pzl_data)
dirs <- grepl("^dir", pzl_data)

path <- c(NULL)
j <- 0
parent_name <- c()
for (i in 1:length(pzl_data)) {
  if (grepl("cd /|cd [a-z]", pzl_data[i])) {
    j <- j + 1
    path[i] <- j
    cur_name <- pzl_data[i] %>% gsub("\\$ cd ","",.)
    parent_name <- paste(parent_name,cur_name, sep = "$")
  }  else if (grepl("cd \\.\\.",pzl_data[i])) {
    j <- j - 1
    path[i] <- j
    parent_name <- stringr::word(parent_name, start = 1, end = -2, sep = fixed("$"))
  } else {
    path[i] <- j
  }
  names(path)[i] <- substr(parent_name,2,nchar(parent_name))
}

# Define parents
names(pzl_data) <- names(path)

# Subset all files and dirs
all_files <- sapply(strsplit(pzl_data[files | dirs]," "),`[[`,1) %>% 
  gsub("dir","0",.) %>% 
  as.numeric(.)

names(all_files) <- names(pzl_data[files | dirs])

# Dirs for file sizes
all_full_paths <- unique(names(all_files))

dir_size <- lapply(all_full_paths, function(x) {

  this_dir_size <- all_files[grepl(x, names(all_files), fixed = TRUE)] %>% 
    sum()
  
  names(this_dir_size) <- x
  
  return(this_dir_size)
}) %>% unlist()


sol1 <- sum(dir_size[dir_size <= 100000])
...
sol2 <- min(dir_size[dir_size > (30000000 - (70000000 - dir_size[[1]]))])
